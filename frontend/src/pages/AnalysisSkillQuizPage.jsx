...
function normalizeQuestionText(text) {
  ...
  if (!raw) return "";
  return raw
    .replace(/^In\s+.+?\s+assessment,\s*/i, "")
    .replace(/^For\s+step\s+'.+?',\s*/i, "")
    .trim();
}
...
export default function AnalysisSkillQuizPage() {
  ...
  const questionCount = Math.max(
    5,
    Math.min(20, Number(location.state?.questionCount || 10)),
  );
  ...
  const analysisRecordId =
    location.state?.analysisRecordId ||
    extractRecordId(returnTo) ||
    restoreState?.analysisRecordId ||
    undefined;
  ...
  const selectedAnswer = currentQuestionId
    ? answersByQuestionId[currentQuestionId]
    : null;
  const answeredCount = questions.filter((question) => {
    const qid = String(question?.question_id || "").trim();
    return qid && Number.isInteger(answersByQuestionId[qid]);
  }).length;
  const questionProgressPct = questions.length
    ? Math.round(((currentIndex + 1) / questions.length) * 100)
    : 0;
  const hasSelection = Number.isInteger(selectedAnswer);
  const aiGuidanceText =
    currentQuestion?.explanation ||
    "Choose the option that best reflects practical and technically accurate reasoning.";
  ...
  async function submitAssessment() {
    ...
    try {
      ...
      setSubmitError(
        err.message || "Could not evaluate this step quiz. Please try again.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <AppShell
      title="Skill Quiz"
      subtitle="Step-by-step checkpoint"
      fullBleed
      darkShell
    >
      <div
        ...
        <main className="mx-auto w-full max-w-6xl px-6 pb-10 pt-3 lg:pb-14 lg:pt-4">
          <div className="mb-2 flex justify-end">
            <button
              type="button"
              onClick={() =>
                navigate(returnTo, { replace: true, state: { restoreState } })
              }
          ...
          {loading && (
            <section className="rounded-[26px] border-t-0 bg-slate-400/10  p-10 text-center">
              ...
              <p className="mt-3 text-sm text-[#a3a8b6] ">
              ...
              <section className="space-y-10 lg:col-span-8">
                <div className="space-y-4">
                  <div className="flex items-end justify-between">
                    ...
                    <span className="text-2xl font-bold text-[#a7a5ff]">
                      {questionProgressPct}%
                    </span>
                  </div>
                ...
                >
                  ...
                  <p className="max-w-2xl text-lg leading-relaxed text-[#a3a8b6]">
                    Think carefully before selecting the most accurate technical
                    answer for {selectedSkill}.
                  </p>
                </div>
                    ...
                    return (
                      ...
                      >
                        <span className="flex items-center gap-6">
                          ...
                          <span
                            className={`text-base font-medium ${isSelected ? "text-white" : "text-[#e3e6f1]"}`}
                          >
                            {option}
                          </span>
                        </span>
                ...
                <div className="rounded-3xl border border-[#a7a5ff1f] bg-[rgba(38,38,38,0.4)] p-6 shadow-[inset_0_1px_0_0_rgba(167,165,255,0.15)]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#9aa0b3]">
                        Skill Booster
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 rounded-xl bg-[#000000] p-4">
                    <p className="text-sm leading-relaxed text-[#a3a8b6]">
                      {aiGuidanceText}
                    </p>
                  </div>

                  <div className="mt-5 space-y-3">
                    <h4 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#9aa0b3]">
                      Related Concepts
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {relatedConcepts.map((concept) => (
                        <span
                          key={concept}
                          className="rounded-full bg-[#262626] px-3 py-1 text-[10px] font-medium text-[#a3a8b6]"
                        >
                          {concept}
                        </span>
                      ))}
          ...
          {!loading && !loadError && phase === "quiz" && !currentQuestion && (
            <section className="rounded-[26px] border border-amber-400/40 bg-amber-500/10 p-6 text-sm text-amber-200">
              Quiz questions are not available for this skill yet. Please go
              back and start the quiz again.
            </section>
          )}
            ...
            <section className="rounded-[26px] border border-[rgba(100,116,139,0.4)]  p-6 text-center shadow-[0_20px_44px_rgba(2,8,24,0.45)]">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-xl border border-slate-700/80 bg-slate-900/50 px-3 py-2">
                  ...
                  <p
                    className={`mt-1 text-lg font-semibold ${result?.passed ? "text-emerald-300" : "text-amber-300"}`}
                  >
              ...
              <p
                className={`mt-3 text-sm ${result?.passed ? "text-emerald-300" : "text-amber-300"}`}
              >
                {result?.motivation || "Assessment completed."}
              </p>
              <p className="mt-2 text-xs uppercase tracking-[0.14em] text-slate-500">
                You can proceed to any roadmap step. Quiz is for confidence
                tracking.
              </p>
