...
export default function AnalysisAssessmentPage() {
  ...
  const [technicalRoleTrack, setTechnicalRoleTrack] = useState(
  ...
  const [resumeRoleTrack, setResumeRoleTrack] = useState(
    "General Professional Role",
  );
  ...
  const technicalAnsweredCount = useMemo(() => {
    return technicalQuestions.filter((item) =>
      Number.isInteger(technicalSelectedAnswers[item.question_id]),
    ).length;
  ...
  async function handleStartTechnicalRound() {
    ...
    try {
      ...
      setError(
        err.message || "Could not generate technical assessment questions.",
      );
    } finally {
  ...
  async function loadResumeRound() {
    ...
    setError("");
    try {
      const payload = await startResumeAssessment(
        analysisRecordId,
        questionCount,
      );
      ...
      setAssessmentStage("round1_result");
      setError(
        err.message || "Could not load resume skills assessment questions.",
      );
    } finally {
  ...
  const canSubmitHr =
    ...
    hrAnsweredCount === hrQuestions.length &&
    !hrSubmitting;

  const technicalCurrentQuestion =
    technicalQuestions[technicalQuestionIndex] || null;
  ...
  const technicalCurrentAnswered = technicalCurrentQuestion
    ? Number.isInteger(
        technicalSelectedAnswers[technicalCurrentQuestion.question_id],
      )
    : false;
  const resumeCurrentAnswered =
    (resumeAnswers[resumeQuestionIndex] || "").trim().length >= 12;
  const hrCurrentAnswered =
    (hrAnswers[hrQuestionIndex] || "").trim().length >= 12;
  ...
  const technicalWrongAnswers =
    technicalEvaluation?.results?.filter((item) => !item.is_correct) || [];
  ...
  const questionCardClass =
    "rounded-2xl border border-[#3a404f] bg-[rgba(16,19,26,0.9)]";
  const stageRoleLabel = assessmentStage.startsWith("round2")
    ? resumeRoleTrack
    : technicalRoleTrack;

  useEffect(() => {
    const isQuestionStage =
      assessmentStage === "round1_questions" ||
      assessmentStage === "round2_questions" ||
      assessmentStage === "round3_questions";

    if (!isQuestionStage) return;
  ...
  return (
    ...
    >
      <div
        ...
        className="relative h-full min-h-0 space-y-6 overflow-x-hidden overflow-y-auto pb-10 pr-1"
      >
        ...
        <div className="relative flex justify-end">
          <button
            type="button"
            onClick={() =>
              navigate("/analysis", { state: { preserveAnalysis: true } })
            }
        ...
        {!analysisRecordId && (
          <div className="rounded-2xl border border-amber-400/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
            Open this page from analysis result so assessment can use your
            uploaded JD.
          </div>
        )}
        ...
        {analysisRecordId && assessmentStage === "setup" && (
          <section
            className={`${glassPanelClass} mx-auto max-w-4xl p-6 md:p-10`}
          >
            <div className="text-center">
              ...
              <p className="mx-auto mt-3 max-w-2xl text-base leading-relaxed text-[#a9abb3]">
                Evaluate your core technical competency through AI-generated
            ...
            <div className="mt-8 rounded-3xl border border-[#45484f33] bg-[rgba(16,19,26,0.85)] p-6 md:p-8">
              <h3 className="text-center text-xl font-bold text-[#ecedf6]">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                ...
                >
                  <p className="text-2xl font-bold text-[#ecedf6]">
                    10 Questions
                  </p>
                  <p className="mt-1 text-sm text-[#a9abb3]">
                    Estimated duration: 15 mins. Focused sprint on core
                    paradigms.
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-[#a9abb3]">
                    Estimated duration: 35 mins. Deep-dive into architectural
                    edge cases.
                  </p>
                </button>
              </div>
            ...
            <div className="mx-auto mt-6 max-w-2xl rounded-2xl border border-[#45484f33] bg-[#10131a] p-4 text-xs text-[#a9abb3]">
              Questions are generated by the Skill Lab engine. Ensure you are in
              a quiet environment.
            </div>
          </section>
        )}
        ...
        {assessmentStage === "round1_questions" &&
          technicalQuestions.length > 0 && (
            <section
              className={`${glassPanelClass} mx-auto max-w-4xl p-5 md:p-7`}
            >
              <div className="mx-auto max-w-3xl">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#a9abb3]">
                      Technical Foundation
                    </p>
                    <p className="mt-1 text-2xl font-semibold text-[#ecedf6]">
                      Question {technicalQuestionIndex + 1} of{" "}
                      {technicalQuestions.length}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-bold text-[#85adff]">
                      {getProgressPercent(
                        technicalQuestionIndex,
                        technicalQuestions.length,
                      )}
                    ...
                    <p className="text-xs uppercase tracking-[0.16em] text-[#a9abb3]">
                      Complete
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#1b2330]">
                  <div
                    ...
                    style={{
                      width: `${getProgressPercent(technicalQuestionIndex, technicalQuestions.length)}%`,
                    }}
                  />
                </div>
                ...
                {technicalCurrentQuestion && (
                  <div className="mt-5 rounded-3xl border border-[#303746] bg-[rgba(14,18,27,0.92)] p-5 md:p-7">
                    ...
                    <div className="mt-6 space-y-3">
                      {(technicalCurrentQuestion.options || []).map(
                        (option, optionIndex) => {
                          const checked =
                            technicalSelectedAnswers[
                              technicalCurrentQuestion.question_id
                            ] === optionIndex;
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                className={`h-6 w-6 rounded-full border-2 ${checked ? "border-[#85adff] bg-[#85adff]" : "border-[#73757d66]"}`}
                              />
                            </button>
                          );
                        },
                      ...
                      <div className="flex items-center gap-2">
                        {technicalQuestionIndex > 0 && (
                          <button
                            type="button"
                            onClick={() =>
                              setTechnicalQuestionIndex((prev) =>
                                Math.max(0, prev - 1),
                              )
                            }
        ...
        {assessmentStage === "round1_result" && technicalEvaluation && (
          <section className={`${glassPanelClass} p-5`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.22em] text-[#85adff]">
                  Round 1 Result: Technical
                </p>
                <h2 className="mt-1 text-2xl font-semibold text-[#ecedf6]">
                  Correct: {technicalEvaluation.correct_count}/
                  {technicalEvaluation.total_questions}
                </h2>
                <p className="mt-1 text-sm text-[#c8cad3]">
                  Score: {technicalEvaluation.score_percentage}%
                </p>
              </div>
            ...
            <div className="mt-4 rounded-2xl border border-[#45484f66] bg-[#10131a] p-3">
              <p className="text-xs uppercase tracking-[0.2em] text-[#a9abb3]">
                ...
                <div className="mt-2 space-y-3">
                  {technicalWrongAnswers.map((item) => (
                    <div
                      ...
                      className="rounded-xl border border-rose-400/40 bg-rose-500/10 p-3"
                    >
        ...
        {(assessmentStage === "round2_loading" ||
          assessmentStage === "round2_questions") &&
          technicalEvaluation && (
            <section
              className={`${glassPanelClass} mx-auto max-w-4xl p-5 md:p-7`}
            >
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.22em] text-[#85adff]">
                    Round 2: Resume Skill Assessment
                  </p>
                  <p className="mt-1 text-sm text-[#c8cad3]">
                    Questions are generated from skills and technologies
                    mentioned in resume.
                  </p>
                  <p className="mt-1 text-xs text-[#d9c8ff]">
                    Detected role track: {resumeRoleTrack}
                  </p>
                </div>
                <p className="text-sm font-medium text-[#ecedf6]">
                  Answered: {resumeAnsweredCount}/{resumeQuestions.length || 0}
                </p>
              </div>
                ...
                resumeCurrentQuestion && (
                  <div className="mx-auto mt-2 max-w-3xl">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#a9abb3]">
                          Resume Skill Application
                        </p>
                        <p className="mt-1 text-2xl font-semibold text-[#ecedf6]">
                          Question {resumeQuestionIndex + 1} of{" "}
                          {resumeQuestions.length}
                        </p>
                      </div>
                      <div className="text-right">
                    ...
                    <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#1b2330]">
                      <div
                        className="h-full rounded-full bg-[linear-gradient(90deg,#85adff,#ac8aff)] transition-all duration-300"
                        style={{
                          width: `${getProgressPercent(resumeQuestionIndex, resumeQuestions.length)}%`,
                        }}
                      />
                    </div>
                    ...
                    <div className="mt-5 rounded-3xl border border-[#303746] bg-[rgba(14,18,27,0.92)] p-5 md:p-7">
                      <p className="text-xs uppercase tracking-[0.2em] text-[#a9abb3]">
                        Focus:{" "}
                        {resumeCurrentQuestion.focus ||
                          "Resume skill application"}
                      </p>
                      <p className="mt-2 text-2xl font-semibold leading-tight text-[#ecedf6]">
                        {resumeCurrentQuestion.question}
                      </p>
                      <textarea
        ...
        {assessmentStage === "round2_result" && resumeEvaluation && (
          <section className={`${glassPanelClass} p-5`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.22em] text-[#85adff]">
                  Round 2 Result: Resume Skills
                </p>
                <h2 className="mt-1 text-2xl font-semibold text-[#ecedf6]">
                  {resumeEvaluation.overall_score}% - {resumeEvaluation.verdict}
                </h2>
                <p className="mt-1 text-xs text-[#d9c8ff]">
                  Evaluation Engine:{" "}
                  {resumeEvaluation.uses_llm
                    ? "LLM + Rule Checks"
                    : "Rule-based fallback"}
                </p>
              </div>
            ...
            <div className="mt-4 space-y-3">
              {(resumeEvaluation.answer_feedback || []).map((item) => (
                <div
                  key={item.question_id}
                  className={`${questionCardClass} p-4`}
                >
                  ...
                  <p className="mt-2 text-xs uppercase tracking-[0.2em] text-[#a9abb3]">
                    Your Answer
                  </p>
                  <p className="mt-1 text-sm text-[#c8cad3]">
            ...
            <div className="mt-4 rounded-2xl border border-emerald-400/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-200">
              Correctness rule for this round: answers with score 70% or above
        ...
        {(assessmentStage === "round3_loading" ||
          assessmentStage === "round3_questions") &&
          resumeEvaluation && (
            <section
              className={`${glassPanelClass} mx-auto max-w-4xl p-5 md:p-7`}
            >
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#c8cad3]">
                    Predefined HR questions asked in real interviews.
                  </p>
                </div>
                <p className="text-sm font-medium text-[#ecedf6]">
                  Answered: {hrAnsweredCount}/{hrQuestions.length || 0}
                </p>
              </div>
                  ...
                  <div className="mx-auto mt-2 max-w-3xl">
                    ...
                    <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#1b2330]">
                      <div
                        className="h-full rounded-full bg-[linear-gradient(90deg,#85adff,#ac8aff)] transition-all duration-300"
                        style={{
                          width: `${getProgressPercent(hrQuestionIndex, hrQuestions.length)}%`,
                        }}
                      />
                    </div>
                    ...
                    <div className="mt-5 rounded-3xl border border-[#303746] bg-[rgba(14,18,27,0.92)] p-5 md:p-7">
                      <p className="text-xs uppercase tracking-[0.2em] text-[#a9abb3]">
                        Focus: {hrCurrentQuestion.focus || "General fit"}
                      </p>
                      <p className="mt-2 text-2xl font-semibold leading-tight text-[#ecedf6]">
                        {hrCurrentQuestion.question}
                      </p>
                      <textarea
                        value={hrAnswers[hrQuestionIndex] || ""}
                        onChange={(event) =>
                          updateHrAnswer(hrQuestionIndex, event.target.value)
                        }
                        rows={6}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {hrQuestionIndex < hrQuestions.length - 1 ? (
                            <button
                              ...
                              onClick={() =>
                                setHrQuestionIndex((prev) =>
                                  Math.min(hrQuestions.length - 1, prev + 1),
                                )
                              }
            ...
            <div className="grid items-start gap-5 xl:grid-cols-[2fr_1fr]">
              <section className={`${glassPanelClass} p-5`}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-[#85adff]">
                      Round 3 Result: HR Interview
                    </p>
                    <h2 className="mt-1 text-2xl font-semibold text-[#ecedf6]">
                      {hrEvaluation.overall_score}% - {hrEvaluation.verdict}
                    </h2>
                    <p className="mt-1 text-xs text-[#d9c8ff]">
                      Evaluation Engine:{" "}
                      {hrEvaluation.uses_llm
                        ? "LLM + Rule Checks"
                        : "Rule-based fallback"}
                    </p>
                  </div>
              ...
              <section className={`${glassPanelClass} p-5`}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-1 text-3xl font-semibold text-[#ecedf6]">
                      {overallScore}%
                    </h2>
                    <p className="mt-1 text-sm text-[#c8cad3]">
                ...
                <div className="mt-4 space-y-3">
                  <div className="rounded-2xl border border-[#45484f66] bg-[#10131a] p-3">
                    ...
                    <p className="mt-1 text-lg font-semibold text-[#ecedf6]">
                      {technicalEvaluation?.score_percentage ?? 0}%
                    </p>
                  </div>
                  <div className="rounded-2xl border border-[#45484f66] bg-[#10131a] p-3">
                    <p className="text-xs uppercase tracking-[0.2em] text-[#a9abb3]">
                    ...
                    <p className="mt-1 text-lg font-semibold text-[#ecedf6]">
                      {resumeEvaluation?.overall_score ?? 0}%
                    </p>
                  </div>
                  <div className="rounded-2xl border border-[#45484f66] bg-[#10131a] p-3">
                    <p className="text-xs uppercase tracking-[0.2em] text-[#a9abb3]">
